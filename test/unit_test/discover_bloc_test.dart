...
void main() {
  ...
  group('DiscoverBloc', () {
    ...
    blocTest<DiscoverBloc, DiscoverState>(
      'emits [DiscoverLoading, DiscoverLoaded] when fetching recipes is successful',
      build: () {
    ...
    blocTest<DiscoverBloc, DiscoverState>(
        'emits [DiscoverLoading, DiscoverError, DiscoverInitial] when fetching recipes throws an error',
        build: () {
          when(mockRecipeRepository.fetchRecipes(any, any))
    ...
    blocTest<DiscoverBloc, DiscoverState>(
      'emits [DiscoverLoading, DiscoverLoaded] when filter is changed',
      build: () {
        when(mockRecipeRepository.fetchRecipes(any, any))
            .thenAnswer((_) async => mockRecipes);
        return discoverBloc;
    ...
    blocTest<DiscoverBloc, DiscoverState>(
      'emits updated state with new query when SearchQueryChanged is added',
      build: () => discoverBloc,
